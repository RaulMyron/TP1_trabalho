...
public class TelaStatusCandidatura extends javax.swing.JFrame {
    ...
    public TelaStatusCandidatura() {
        initComponents();
        this.Controller = new CandidatoController(); // <-- ADICIONE ESTA
        popularTabela(); // <-- E ESTA CHAMADA DE MÉTODO
    }
    
    private void popularTabela() {
    // 1. Pega a lista de candidatos do controller.
    List<Candidato> listaCandidatos = Controller.getListaCandidatos();

    // 2. Pega o "modelo" da nossa tabela. É através dele que manipulamos os dados.
    // Troque "jTableStatus" pelo nome real da variável da sua tabela.
    DefaultTableModel model = (DefaultTableModel) jTableStatus.getModel();

    // 3. Limpa a tabela para garantir que não haja linhas antigas.
    model.setRowCount(0);

    // 4. Percorre a lista de candidatos e adiciona cada um como uma nova linha na tabela.
    for (Candidato c : listaCandidatos) {
        // Cria um array de objetos para representar a linha.
        // A ORDEM DOS ITENS AQUI DEVE SER A MESMA DAS COLUNAS DA SUA TABELA.
        Object[] rowData = {
            c.getNome(),
            c.getCpf(),     // Vamos usar o CPF no lugar de "Vaga Aplicada" por enquanto
            c.getEmail(),   // Vamos usar o Email no lugar de "Data" por enquanto
            c.getFormacao() // Vamos usar a Formação no lugar de "Status" por enquanto
        };
